...
export class Map {
    ...
    setInfoWindow(location) {
        ...
        return `<div class="puntos-instalacion__map__info-window">
            <button class="puntos-instalacion__map__info-window__close"><span class="alk-icon-close"></span></button>
            <h4>${location.name}</h4>
    ...
    async setMarkers(locationPoints) {
        ...
        locationPoints.map(location => {
            const marker = new google.maps.Marker({
                ...
                map: this.map,
                icon: `https://cdn.jsdelivr.net/gh/ux-alkosto/puntos-instalacion-llantas/dist/${this.baseSite}/img/pin.svg`,
                title: location.name
            });
